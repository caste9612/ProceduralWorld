...
window.addEventListener('DOMContentLoaded', function(){
    ...
    var createScene = function() {
        ...
        var mapSubX = 1500;             // map number of points on the width
        var mapSubZ = 1500;              // map number of points on the height
        ...
        for (var l = 0; l < mapSubZ; l++) {                 // loop on height points
            for (var w = 0; w < mapSubX; w++) {
                var x = (w - mapSubX * 0.5);          // distance inter-points = 5 on the width
                var z = (l - mapSubZ * 0.5);          // distance inter-points = 2 on the width
                var e =  5 * noise.perlin2 (0.01 * x, 0.01 * z) + 0.5 * noise.perlin2(0.1 * x, 0.1*z) + 0.05 * noise.perlin2(0.9 * x, 0.9 * z);  // let's increase a bit the noise computed altitude
        ...
        skyMaterial.luminance = 0.9;// Manually set the sun position
        ...
        terrain.LODLimits = [1, 1, 2,2,2,4,4,8,8,8,8];        // Terrain camera LOD : custom function
